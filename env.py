...
class Env(object):
    ...
    def load_items(self, item_ids):
        ...
        for count in range(len(item_ids)):
            file_dir = self.obj_info['dir'][item_ids[count]]
    ...
    def pack_item(self, item_id, transform):
        ...
        orientation_close = np.linalg.norm(curr_euler - target_euler) < 0.35
        stability = np.linalg.norm(new_pos - curr_pos) < 0.02 and orientation_close
        return stability
