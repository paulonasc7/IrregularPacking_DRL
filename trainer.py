...
class Trainer(object):
    ...
    def forward(self, color_heightmap, depth_heightmap, is_volatile=False, specific_rotation=-1):
        ...
        if self.method == 'reactive':
            ...
            crop_start = int(padding_width/2)
            crop_end = int(color_heightmap_2x.shape[0]/2 - padding_width/2)
            for rotate_idx in range(len(output_prob)):
                if rotate_idx == 0:
                    push_predictions = F.softmax(output_prob[rotate_idx][0], dim=1).cpu().data.numpy()[:,0,crop_start:crop_end,crop_start:crop_end]
                    grasp_predictions = F.softmax(output_prob[rotate_idx][1], dim=1).cpu().data.numpy()[:,0,crop_start:crop_end,crop_start:crop_end]
                else:
                    push_predictions = np.concatenate((push_predictions, F.softmax(output_prob[rotate_idx][0], dim=1).cpu().data.numpy()[:,0,crop_start:crop_end,crop_start:crop_end]), axis=0)
                    grasp_predictions = np.concatenate((grasp_predictions, F.softmax(output_prob[rotate_idx][1], dim=1).cpu().data.numpy()[:,0,crop_start:crop_end,crop_start:crop_end]), axis=0)
